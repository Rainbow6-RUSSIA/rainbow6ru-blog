...
const Link = ({ platform, name, children, ...args }) => {
  const url =
    platform === 'twitch'
      ...
      : ''
  return (
    <a className={args.className} href={url}>
      {children}
    </a>
  )
}
...
const Icon = ({ platform }) => {
  return platform === 'twitch' ? (
    <FontAwesomeIcon
      style={{ color: '#6441A4', height: '1em', width: '1em' }}
      icon={faTwitch}
    />
  ) : platform === 'mixer' ? (
    <FontAwesomeIcon style={{ height: '1em', width: '1em' }} icon={faMixer} />
  ) : null
}
...
async function fetchMixer(channels) {
  ...
  const res = await fetch(
    `https://mixer.com/api/v1/channels?fields=token,online,bannerUrl,viewersCurrent,type&where=token:in:${channels.join(';')}`,
    {
...
async function fetchTwitch(channels) {
  ...
  const res = await fetch(
    `https://api.twitch.tv/kraken/streams/?channel=${channels.join(',')}`,
    {
      headers: {
...
const reducer = (state, action) => {
  switch (true) {
    case typeof action === 'string':
      return state.map((i) =>
        i.platform === action ? { ...i, fetched: true } : i
      )
    case Array.isArray(action):
      return state.map((i) => ({
        ...i,
        ...action.find((r) => r.name === i.name && r.platform === i.platform),
      }))
  }
}
...
const Streams = ({ items }) => {
  ...
  useEffect(() => {
    import('./Player').then((module) => {
      ...
      fetchMixer(state)
        .then(dispatch)
        .finally(() => dispatch('mixer'))
      fetchTwitch(state)
        .then(dispatch)
        .finally(() => dispatch('twitch'))
    })
  }, [])
