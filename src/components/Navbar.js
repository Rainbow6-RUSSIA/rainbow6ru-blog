  ...
  constructor(props) {
    ...
    const active = Boolean(
      typeof localStorage !== `undefined` &&
        localStorage?.getItem('burger-menu')
    )
    this.state = {
      active,
    ...
    return (
      <nav
        className={`navbar is-fixed-top ${
          this.props.onTop ? 'navbar-top' : 'navbar-scrolled'
        }`}
        role="navigation"
        ...
        >
          <div className="navbar-start">
            ...
            <Link className="navbar-item" to="/about">
              <FontAwesomeIcon icon={faInfoCircle} />О нас
            </Link>
          </div>
          <div className="navbar-end">
            ...
            >
              <FontAwesomeIcon
                icon={this.state.loggedIn ? faDoorOpen : faSignInAlt}
              />
              {this.state.loggedIn
                ? auth.user
